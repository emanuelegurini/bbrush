(function initBbrushPluginManifest() {
  if (
    window.__BBRUSH_PLUGIN_IDS__ &&
    window.__BBRUSH_PLUGIN_MANIFEST__ &&
    window.__BBRUSH_REGISTER_PLUGIN_IMPL__
  ) {
    return;
  }
  ...
  const frozenPluginManifest = freezeDeep(pluginManifest);
  const pluginImplementations =
    window.__BBRUSH_PLUGIN_IMPLEMENTATIONS__ &&
    typeof window.__BBRUSH_PLUGIN_IMPLEMENTATIONS__ === 'object'
      ? window.__BBRUSH_PLUGIN_IMPLEMENTATIONS__
      : {};

  window.__BBRUSH_PLUGIN_IDS__ = PLUGIN_IDS;
  window.__BBRUSH_PLUGIN_MANIFEST__ = frozenPluginManifest;
  window.__BBRUSH_PLUGIN_IMPLEMENTATIONS__ = pluginImplementations;
  window.__BBRUSH_REGISTER_PLUGIN_IMPL__ = function registerBbrushPluginImplementation(
    pluginId,
    implementation
  ) {
    if (typeof pluginId !== 'string' || !implementation || typeof implementation !== 'object') {
      return;
    }

    pluginImplementations[pluginId] = { ...implementation };
  };
})();
